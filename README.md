...
To communicate with RFM12BS software version if SPI is implemented using [mraa library][] memory mapped io. Kernel version of SPI is not supported at the moment because of very slow user/kernel space data transfer.
...
int wait_for_low(mraa_gpio_context pin, uint32_t timeout)
{
	...
	while(mraa_gpio_read(pin)) {
		// millis returns number of milliseconds tstamp and now
		dt = millis(tstamp); 
		if (dt > timeout)
			return -1;
	}
